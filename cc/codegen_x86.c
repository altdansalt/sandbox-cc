...
// x86-64 backend: upstream chibicc's codegen, emitting into a buffer that
// asm.c assembles straight into a static ELF executable.
static char *asm_buf;
static size_t asm_len, asm_cap;
static int depth;
...
static void println(char *fmt, ...) {
  ...
  for (;;) {
    va_start(ap, fmt);
    int n = vsnprintf(asm_buf + asm_len, asm_cap - asm_len, fmt, ap);
    va_end(ap);
    if (n < 0) error("vsnprintf failed");
    if (asm_len + n + 2 <= asm_cap) { asm_len += n; asm_buf[asm_len++] = '\n'; asm_buf[asm_len] = 0; return; }
    asm_cap = asm_cap ? asm_cap * 2 : 1 << 20;
    if (asm_cap < asm_len + n + 2) asm_cap = asm_len + n + 2;
    asm_buf = realloc(asm_buf, asm_cap);
  }
}
...
// align_to is defined in codegen.c
...
static void gen_expr(Node *node) {
  ...
  case ND_NUM: {
    ...
    println("  mov $%" PRId64 ", %%rax", node->val);
    return;
  }
  case ND_NEG:
    gen_expr(node->lhs);
  ...
  case ND_ASSIGN:
    ...
    if (node->lhs->kind == ND_MEMBER && node->lhs->member->is_bitfield) {
      ...
      println("  mov %%rax, %%rdi");
      println("  and $%" PRId64 ", %%rdi", (1LL << mem->bit_width) - 1);
      println("  shl $%d, %%rdi", mem->bit_offset);

      println("  mov (%%rsp), %%rax");
      load(mem->ty);

      int64_t mask = ((1LL << mem->bit_width) - 1) << mem->bit_offset;
      println("  mov $%" PRId64 ", %%r9", ~mask);
      println("  and %%r9, %%rax");
      println("  or %%rdi, %%rax");
      store(node->ty);
      println("  mov %%r8, %%rax");
...
static void gen_stmt(Node *node) {
  ...
  case ND_SWITCH:
    ...
    for (Node *n = node->case_next; n; n = n->case_next) {
      ...
      if (n->begin == n->end) {
        println("  cmp $%" PRId64 ", %s", n->begin, ax);
      ...
      println("  sub $%" PRId64 ", %s", n->begin, di);
      println("  cmp $%" PRId64 ", %s", n->end - n->begin, di);
...
void codegen_x86(Obj *prog, FILE *out) {
  ...
  if (opt_S) { fwrite(asm_buf, 1, asm_len, out); return; }
  assemble_elf(asm_buf, out);
}

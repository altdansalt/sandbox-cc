...
static int64_t bss_len;
static Sec *cur;               // current section, or NULL for .bss
static bool cur_bss;

typedef struct { int sec; int64_t off; } Sym;  // sec: 0 text, 1 data, 2 bss
static HashMap syms;

typedef struct { int sec; int64_t off; char *name; int64_t addend; bool abs64; } Fixup;
static Fixup *fixups;
...
static void add_fixup(char *name, int64_t addend, bool abs64) {
...
static int64_t num(char *s, int64_t lo, int64_t hi) {   // strict decimal/hex integer within [lo, hi]
  char *end;
  errno = 0;
  s = trim(s);
...
static void directive(char *d, char *rest) {
  ...
  if (!strcmp(d, ".zero")) { int64_t v = num(rest, 0, 1LL << 31); if (cur_bss) bss_len += v; else for (int64_t i = 0; i < v; i++) emit8(0); return; }
  if (!strcmp(d, ".quad")) {
    rest = trim(rest);
    ...
    int64_t addend = 0;
    if (plus) { addend = num(plus, -(1LL << 40), 1LL << 40); *plus = 0; }

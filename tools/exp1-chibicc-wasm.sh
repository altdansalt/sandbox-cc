...
WZ="wazero run -mount=/usr/include:/usr/include:ro -mount=$ROOT/chibicc:/chibicc:ro -mount=$ROOT/$OUT:/out"
pass=0; fail=0; same=0; differ=0
for src in chibicc/test/*.c; do
  n=$(basename $src .c)
  ./chibicc/chibicc -Ichibicc/include -Ichibicc/test -S -o $OUT/$n.native.s $src 2>$OUT/$n.native.err
  $WZ build/chibicc/chibicc.wasm -I/chibicc/include -I/chibicc/test -S -o /out/$n.wasm.s /chibicc/test/$n.c 2>$OUT/$n.wasm.err
